...
여기서 {::nomarkdown}\(\hat{s}_\theta(w;l) := \beta \log \frac{\pi_\theta(y_w \mid x)}{\pi_\theta(y_l \mid x)}\){:/nomarkdown} 이다.

문제는 이 loss의 최적해가 {::nomarkdown}\(\pi_\theta(y_l \mid x) \to 0\){:/nomarkdown}이면 {::nomarkdown}\(\pi_\theta(y_w \mid x)\){:/nomarkdown}의 값에 관계없이 달성될 수 있다는 점이다. 즉, **rejected response의 확률만 줄이면 loss가 줄어들기 때문에**, 모델이 chosen response의 확률을 높이는 방향으로 학습하지 않게 된다.

Gradient 관점에서 보면, {::nomarkdown}\(\pi_\theta(y_l \mid x)\){:/nomarkdown}에 대한 gradient에 {::nomarkdown}\(\frac{1}{\pi_\theta(y_l \mid x)}\){:/nomarkdown} 항이 포함되어 있어, rejected response의 확률이 작아질수록 gradient가 폭발적으로 커진다. 반면 chosen response에 대한 gradient는 상대적으로 안정적이어서, **최적화가 rejected response 쪽으로 편향**된다.

## Loss Contour 분석

아래 그림은 {::nomarkdown}\(\pi_{\text{ref}}(y_w \mid x) = 0.4\){:/nomarkdown}, {::nomarkdown}\(\pi_{\text{ref}}(y_l \mid x) = 0.1\){:/nomarkdown}일 때 각 방법의 loss contour를 보여준다.
...
- **DPO**: {::nomarkdown}\(\pi_\theta(y_l \mid x)\){:/nomarkdown}를 0에 가깝게 줄이는 것만으로 loss가 최소화됨. {::nomarkdown}\(\pi_\theta(y_w \mid x)\){:/nomarkdown}의 값은 무관
- **DPOP**: DPO와 유사한 패턴. Penalty는 {::nomarkdown}\(\pi_\theta(y_w \mid x)\){:/nomarkdown}가 매우 낮을 때만 활성화
- **DPO+NLL**: 개선되지만, {::nomarkdown}\(\alpha\){:/nomarkdown} 값에 따라 contour가 극적으로 변화
- **BDPO**: {::nomarkdown}\(\pi_\theta(y_w \mid x)\){:/nomarkdown}를 높이고 {::nomarkdown}\(\pi_\theta(y_l \mid x)\){:/nomarkdown}를 낮추는 방향 모두에 대해 균형 잡힌 최적화
...
Chosen response의 확률을 직접 높이려는 시도이지만, **{::nomarkdown}\(\alpha\){:/nomarkdown} 값에 극도로 민감**하다. 아래 그림에서 {::nomarkdown}\(\alpha\){:/nomarkdown} 값에 따른 loss contour 변화를 확인할 수 있다.
...
{::nomarkdown}\(\alpha = 0.01\){:/nomarkdown}이면 DPO와 거의 동일하고, {::nomarkdown}\(\alpha = 10\){:/nomarkdown}이면 극단적인 loss 패턴이 나타나 reference model에서 크게 벗어난다. 실험에서 {::nomarkdown}\(\alpha\){:/nomarkdown}를 0.01부터 10까지 탐색했지만, 어떤 값에서도 BDPO를 넘지 못했다.
...
핵심 아이디어는 단순하다. DPO loss의 분모에서 {::nomarkdown}\(\pi_\theta(y_l \mid x)\){:/nomarkdown}를 **mixture distribution**으로 대체한다:

{::nomarkdown}
$$\pi_{\text{mix}}(y \mid x) = \lambda \pi_\theta(y \mid x) + (1-\lambda)\pi_{\text{ref}}(y \mid x)$$
{:/nomarkdown}

이렇게 하면 분모가 {::nomarkdown}\((1-\lambda)\pi_{\text{ref}}(y_l \mid x)\){:/nomarkdown}로 lower bound되어, rejected response의 확률이 0에 가까워져도 gradient가 폭발하지 않는다.
...
where {::nomarkdown}\(\hat{s}_\theta(w;l) := \beta \log \frac{\pi_\theta(y_w \mid x)}{\pi_\theta(y_l \mid x)}\){:/nomarkdown}.

The problem is that the optimal solution can be achieved as long as {::nomarkdown}\(\pi_\theta(y_l \mid x) \to 0\){:/nomarkdown}, regardless of {::nomarkdown}\(\pi_\theta(y_w \mid x)\){:/nomarkdown}. Since **reducing the rejected response probability alone decreases the loss**, the model never learns to increase the chosen response probability.

From a gradient perspective, the gradient w.r.t. {::nomarkdown}\(\pi_\theta(y_l \mid x)\){:/nomarkdown} contains a {::nomarkdown}\(\frac{1}{\pi_\theta(y_l \mid x)}\){:/nomarkdown} term that explodes as the rejected probability approaches zero, while the chosen response gradient remains stable. This creates an **optimization bias toward the rejected response**.

## Loss Contour Analysis

The figure below shows loss contours for each method with {::nomarkdown}\(\pi_{\text{ref}}(y_w \mid x) = 0.4\){:/nomarkdown} and {::nomarkdown}\(\pi_{\text{ref}}(y_l \mid x) = 0.1\){:/nomarkdown}.

<img src="/assets/images/rethinking-dpo/fig1_loss_contour.png" alt="Loss contour comparison of DPO, DPOP, DPO+NLL, and BDPO" style="width:100%; margin: 1.5rem 0;">

- **DPO**: Loss is minimized by reducing {::nomarkdown}\(\pi_\theta(y_l \mid x)\){:/nomarkdown} to near zero alone; {::nomarkdown}\(\pi_\theta(y_w \mid x)\){:/nomarkdown} is irrelevant
- **DPOP**: Similar pattern to DPO; penalty only activates when {::nomarkdown}\(\pi_\theta(y_w \mid x)\){:/nomarkdown} is very low
- **DPO+NLL**: Improved, but contours change dramatically with α
- **BDPO**: Balanced optimization toward both increasing {::nomarkdown}\(\pi_\theta(y_w \mid x)\){:/nomarkdown} and decreasing {::nomarkdown}\(\pi_\theta(y_l \mid x)\){:/nomarkdown}
...
The core idea is simple. We replace {::nomarkdown}\(\pi_\theta(y_l \mid x)\){:/nomarkdown} in the DPO loss denominator with a **mixture distribution**:

{::nomarkdown}
$$\pi_{\text{mix}}(y \mid x) = \lambda \pi_\theta(y \mid x) + (1-\lambda)\pi_{\text{ref}}(y \mid x)$$
{:/nomarkdown}

This lower-bounds the denominator by {::nomarkdown}\((1-\lambda)\pi_{\text{ref}}(y_l \mid x)\){:/nomarkdown}, preventing gradient explosion even as the rejected probability approaches zero.
