...
- **Transition probability** \(P\_\sigma\)가 noise distribution을 통해 재정의된다
- **Reward function** \(R\_\sigma\)에 noise 크기만큼의 penalty가 내장된다
...
1. **Uniform mixture**: \(q\_t^A(a' \mid a) = \alpha(t) \cdot U(a' \mid A) + (1-\alpha(t)) \cdot q\_t(a' \mid a)\)
...
- **Transition probabilities** \(P\_\sigma\) are redefined through the noise distribution
- **Reward function** \(R\_\sigma\) has the noise penalty baked in
