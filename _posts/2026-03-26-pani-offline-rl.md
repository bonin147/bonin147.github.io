...
{::nomarkdown}\(a'\){:/nomarkdown}은 dataset action {::nomarkdown}\(a\){:/nomarkdown}에 noise를 주입한 perturbed action이다. 끝이다. **이게 전부다.**
...
- **Transition probability** {::nomarkdown}\(P_\sigma\){:/nomarkdown}가 noise distribution을 통해 재정의된다
- **Reward function** {::nomarkdown}\(R_\sigma\){:/nomarkdown}에 noise 크기만큼의 penalty가 내장된다
...
1. **Uniform mixture**: {::nomarkdown}\(q_t^A(a' \mid a) = \alpha(t) \cdot U(a' \mid A) + (1-\alpha(t)) \cdot q_t(a' \mid a)\){:/nomarkdown}
...
OOD overestimation probability {::nomarkdown}\(P(Q(s, a') > Q(s, a))\){:/nomarkdown} ({::nomarkdown}\(a\){:/nomarkdown}는 dataset action, {::nomarkdown}\(a'\){:/nomarkdown}은 uniform sample):
...
- **Penalty term {::nomarkdown}\(\|a - a'\|_2^2\){:/nomarkdown}의 scale 문제** — reward scale과 action scale이 다른 task에서 penalty의 상대적 크기가 달라질 수 있다. Scaling hyperparameter {::nomarkdown}\(\alpha\){:/nomarkdown}의 도입이 필요할 수 있다
...
Where {::nomarkdown}\(a'\){:/nomarkdown} is a noise-perturbed version of the dataset action {::nomarkdown}\(a\){:/nomarkdown}. That's it. **That's the whole method.**
...
- **Transition probabilities** {::nomarkdown}\(P_\sigma\){:/nomarkdown} are redefined through the noise distribution
- **Reward function** {::nomarkdown}\(R_\sigma\){:/nomarkdown} has the noise penalty baked in
...
Probability of OOD overestimation {::nomarkdown}\(P(Q(s, a') > Q(s, a))\){:/nomarkdown} (where {::nomarkdown}\(a\){:/nomarkdown} is a dataset action, {::nomarkdown}\(a'\){:/nomarkdown} is uniformly sampled):
...
- **The penalty term {::nomarkdown}\(\|a - a'\|_2^2\){:/nomarkdown} scale issue** — different tasks have different reward and action scales, potentially making the penalty ineffective or overwhelming. A scaling hyperparameter {::nomarkdown}\(\alpha\){:/nomarkdown} may be needed
