...
<span>\(a'\)</span>은 dataset action <span>\(a\)</span>에 noise를 주입한 perturbed action이다. 끝이다. **이게 전부다.**
...
- **Transition probability** <span>\(P_\sigma\)</span>가 noise distribution을 통해 재정의된다
- **Reward function** <span>\(R_\sigma\)</span>에 noise 크기만큼의 penalty가 내장된다
...
1. **Uniform mixture**: <span>\(q_t^A(a' \mid a) = \alpha(t) \cdot U(a' \mid A) + (1-\alpha(t)) \cdot q_t(a' \mid a)\)</span>
...
OOD overestimation probability <span>\(P(Q(s, a') > Q(s, a))\)</span> (<span>\(a\)</span>는 dataset action, <span>\(a'\)</span>은 uniform sample):
...
- **Penalty term <span>\(\|a - a'\|_2^2\)</span>의 scale 문제** — reward scale과 action scale이 다른 task에서 penalty의 상대적 크기가 달라질 수 있다. Scaling hyperparameter <span>\(\alpha\)</span>의 도입이 필요할 수 있다
...
Where <span>\(a'\)</span> is a noise-perturbed version of the dataset action <span>\(a\)</span>. That's it. **That's the whole method.**
...
- **Transition probabilities** <span>\(P_\sigma\)</span> are redefined through the noise distribution
- **Reward function** <span>\(R_\sigma\)</span> has the noise penalty baked in
...
Probability of OOD overestimation <span>\(P(Q(s, a') > Q(s, a))\)</span> (where <span>\(a\)</span> is a dataset action, <span>\(a'\)</span> is uniformly sampled):
...
- **The penalty term <span>\(\|a - a'\|_2^2\)</span> scale issue** — different tasks have different reward and action scales, potentially making the penalty ineffective or overwhelming. A scaling hyperparameter <span>\(\alpha\)</span> may be needed
